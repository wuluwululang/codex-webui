...
# 创建只能访问一个项目目录的 token
npm run token -- add phone --label "My phone" --cwd "E:\MyProject"

# 一个 token 可允许多个目录
npm run token -- add tablet --cwd "E:\ProjectA" --cwd "E:\ProjectB"

# 生成完整访问地址与二维码（会显示密钥）
npm run token -- qr phone

# 轮换、停用或删除
npm run token -- rotate phone
npm run token -- disable phone
npm run token -- remove phone --yes

# 查看每个访问 token 的使用情况
npm run token -- stats
npm run token -- stats phone
```

运行中的服务会自动重新加载 token 变更。轮换、停用或删除后，旧连接会在下一次请求时断开。

仓库同时附带 [`codex-mobile-token-manager`](skills/codex-mobile-token-manager/SKILL.md) skill。Codex 可以据此处理“给项目 A 新增一个手机 token”“显示二维码”“查看 token 使用统计”等自然语言请求。
