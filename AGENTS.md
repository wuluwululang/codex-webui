...
2. Run `npm install`.
3. Run `npm run setup` to verify Codex availability and initialize the local token store.
4. Run `npm test`.
...
Use the repository skill at `skills/codex-mobile-token-manager/SKILL.md`. Do not edit `tokens.json` directly and do not expose raw tokens unless the user explicitly requests a URL or QR code.
